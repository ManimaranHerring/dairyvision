...
from farmers.models import Farmer, Farm, Cattle, Cooperative
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        coop = Cooperative.objects.create(
            name='NABARD MABIF Madurai Cooperative',
            code='MADURAI1',
            district='Madurai',
            state='Tamil Nadu',
            plan='growth',
        ...
        for phone, name, village, role in farmers_data:
            ...
            f = Farmer.objects.create(
                user=u, cooperative=coop, phone=phone,
                village=village, district='Madurai', role=role)
            farmers.append(f)
        ...
        manager = Farmer.objects.create(
            user=mu, cooperative=coop, phone='9000000001',
        ...
        for i, (tag, breed, age, milking, exp_yield) in enumerate(tags):
            f = farmers[i % 5]
            c = Cattle.objects.create(
                farmer=f, tag_number=tag, breed=breed,
                age_years=age, is_milking=milking,
                expected_daily_yield_litres=Decimal(str(exp_yield)))
        ...
        for farmer, name, crop, area, lat, lng in farms_data:
            fm = Farm.objects.create(
                ...
                area_acres=Decimal(str(area)), latitude=lat, longitude=lng,
        ...
        for farm in farms:
            ...
            for i in range(14):
                ...
                ndvi = round(max(0.05, min(0.80, base+random.uniform(-0.08,0.08))), 3)
        ...
        CropAlert.objects.create(
            farm=farms[6], alert_type='stress', severity='high',
        ...
        CropAlert.objects.create(
            farm=farms[5], alert_type='stress', severity='medium',
            message_en=f"Moderate stress on '{farms[5].name}'. NDVI=0.35. Monitor water levels.",
            message_ta=f"'{farms[5].name}' நிலத்தில் மிதமான அழுத்தம். NDVI=0.35. நீர் நிலையை கண்காணிக்கவும்.",
            is_resolved=False,
        )
        ...
        for i, gl in enumerate(grazing_lands):
            for d in range(7):
                ...
                ndvi = round(pasture_ndvi[i]+random.uniform(-0.05,0.05), 3)
                PastureReading.objects.create(
                    grazing_land=gl, reading_date=rdate,
                    ndvi_value=Decimal(str(ndvi)),
        ...
        # ── Market Listings ───────────────────────────────────────
        listings_data = [
            ('Pure Cow Ghee — Madurai Cooperative','ghee',
             '100% pure cow ghee from grass-fed cows in Madurai district.',580,'kg',25,1,batches[0],'FSSAI certified'),
            ('Fresh Paneer — Daily Production','paneer',
             'Fresh paneer made daily from full-fat milk.',280,'kg',40,2,batches[1],'FSSAI certified'),
            ('Curd / Yogurt — Traditional Style','curd',
             'Set curd made from full-fat cow milk. No preservatives.',60,'kg',60,5,batches[2],''),
            ('White Butter — Unsalted','butter',
             'Traditional white butter made from cream.',320,'kg',20,2,batches[3],'FSSAI certified'),
            ('Premium Cow Ghee — Export Quality','ghee',
             'Premium ghee from HF and Jersey cattle. Export quality.',650,'kg',30,5,batches[4],'FSSAI, Organic certified'),
        ]
        listings = []
        for name,ptype,desc,price,unit,qty,min_ord,batch,cert in listings_data:
            l = ProductListing.objects.create(
                cooperative=manager,
                product_name=name, product_type=ptype, description=desc,
                price_per_unit=Decimal(str(price)), unit=unit,
                quantity_available=Decimal(str(qty)),
                minimum_order=Decimal(str(min_ord)),
                source_batch=batch, district='Madurai',
                status='active', certifications=cert,
            )
            listings.append(l)

        orders_data = [
            (listings[0],'Hotel Madurai Residency','9444111222','hotel','Madurai',5,'confirmed'),
            (listings[1],'Star Supermarket Chennai','9444333444','supermarket','Chennai',20,'pending'),
            (listings[2],'Government School Madurai','9444555666','school','Madurai',30,'confirmed'),
            (listings[4],'Gulf Exports Pvt Ltd','9444777888','export','Chennai',15,'pending'),
        ]
        for listing,bname,bphone,btype,bcity,qty,status in orders_data:
            Order.objects.create(
                listing=listing, buyer_name=bname, buyer_phone=bphone,
                buyer_type=btype, buyer_city=bcity,
        ...
        self.stdout.write(f'Cooperative join code: {coop.code}')
